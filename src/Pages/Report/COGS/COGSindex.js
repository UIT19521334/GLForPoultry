...
import { Api_Export_COGS_New, Api_PDF_Report_COGS_New } from '~/components/Api/Report';
...
export default function Report_COGS({ title }) {
    ...
    const dataCostCenter = useSelector((state) => {
        const filteredData = state.FetchApi.listData_CostCenter.filter((data) => data.kind_of_location === 'SH');
        return [{ code: null, name: 'Tất cả' }, ...filteredData];
    });
    const [valueDateAccountPeriod, setValueDateAccountPeriod] = React.useState(dayjs(dataPeriod_From_Redux));
    const [valueCostCenter, setValueCostCenter] = React.useState(null);
    ...
    useEffect(() => {
        if (callApi) {
            const process = async () => {
                ...
                const status_code = await Api_Export_COGS_New({
    ...
    useEffect(() => {
        if (callApi) {
            const process = async () => {
                setIsLoading(true);
                const status_code = await Api_PDF_Report_COGS_New({
                    COSTCENTER: valueCostCenter,
                    PERIOD_MONTH: valueDateAccountPeriod.month() + 1,
                    PERIOD_YEAR: valueDateAccountPeriod.year(),
